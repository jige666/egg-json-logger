...
util.getFileLine = function (name, layer) {
  let path;
  try {
    path =  name + '/' + new Error().stack.split('\n')[layer].split(name + '/')[1].split(':').slice(0, 2).join(':');
  }
  catch (e){
    console.error('getFileLine error', e)
    path = 'unknow'
  }
  return path
}